 ...
 * @prop {Object} options Collection options
 * @prop {Number} options.limit Max number of items to hold
 * @prop {Function?} options.filter Filter function for caching items
 * @prop {Array<string>?} options.excludeProperties Properties to exclude when adding items
 */
class Collection extends Map {
  /**
   * Construct a Collection
   * @arg {Class} baseObject The base class for all items
   * @arg {Object} [options] Collection options
   * @arg {Number} [options.limit] Max number of items to hold
   * @arg {Function?} [options.filter] Filter function for caching items
   * @arg {Array<string>?} [options.excludeProperties] Properties to exclude when adding items
   */
  constructor(baseObject, options = {}) {
    ...
    this.options = {
      ...
      excludeProperties: options.excludeProperties || null,
    };
  }
  ...
  add(obj, extra, replace) {
    ...
    if (this.options.filter && !this.options.filter(obj)) {
      return obj;
    }

    if (this.options.excludeProperties) {
      for (const prop of this.options.excludeProperties) {
        delete obj[prop];
      }
    }
